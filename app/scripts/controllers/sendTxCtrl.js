...
var sendTxCtrl = function ($scope, $sce, walletService) {
    ...
    $scope.scaduleTx = function () {
        ...
        circle = setInterval(() => {
            ajaxReq.getCurrentBlock(function (data) {
                ...
                console.log($scope.currentBlockNumber)
            });
        ...
        console.log('starting auto send...')

    }

    $scope.stopScaduleTx = function () {
    ...
    $scope.sendDonate = function (){
        $scope.tx.to = "0x06c6A0E33171d6aEa0a9E057B17CC033F9D61dA1"
        $scope.tx.value = "1";
        if (!$scope.Validator.isValidAddress($scope.tx.to)) {
            //if (!ethFuncs.validateEtherAddress($scope.tx.to)) {
            $scope.notifier.danger(globalFuncs.errorMsgs[5]);
